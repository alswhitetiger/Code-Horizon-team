  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {recommendations.map(r => (
        <div key={r.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow cursor-pointer">
          <Badge variant={r.type === '보충학습' ? 'danger' : r.type === '개념복습' ? 'warning' : 'default'}>{r.type}</Badge>
          <h3 className="font-medium mt-2 mb-1">{r.title}</h3>
